  ...
  (
    ...
    return (
      <div
        ref={ref}
        className={cn(
            "group/sidebar hidden md:flex flex-col h-screen text-card-foreground transition-[width] duration-300 ease-in-out",
            state === "expanded" ? "w-[--sidebar-width]" : "w-[--sidebar-width-icon]",
            variant === 'sidebar' && 'border-r bg-card',
            className
        )}
        data-state={state}
...
>(({ className, ...props }, ref) => {
  return (
    <Input
      ...
      data-sidebar="input"
      className={cn(
        "h-8 w-full bg-transparent shadow-none focus-visible:ring-2 focus-visible:ring-ring",
        className
      )}
      {...props}
    />
